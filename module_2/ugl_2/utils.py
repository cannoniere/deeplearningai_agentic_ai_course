...
USER = "aldo"
PASSWORD = "USNavy##01" # Replace with your PostgreSQL password
HOST = "localhost"
PORT = "5432"

connection_params_postgres = f"dbname=postgres user={USER} password={PASSWORD} host={HOST} port={PORT}"
connection_params_new_db = f"dbname={DB_NAME} user={USER} password={PASSWORD} host={HOST} port={PORT}"
...
def create_transactions_db(
    ...
    """
    Create a PostgreSQL DB with a single 'transactions' table (event-sourced).
    All analytics must be derived from this table (no views).
    """
    conn = psycopg.connect(connection_params_postgres, autocommit=True)
    cur = conn.cursor()

    # 1. Connect to the default 'postgres' database with autocommit enabled
    #    to create the new database.
    # Drop the database if it exists (optional, for rerunning the script)
    try:
